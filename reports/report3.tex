...
\lstinputlisting[language=SQL]{../scripts/entities.sql}
...
Note-se que a criação dos índices está ordenada tal e qual a
...
\begin{itemize}
  ...
  \item Tanto \texttt{category}, em \texttt{Product}, como \texttt{name}, em \texttt{has\_category},
        são indexadas via \textbf{BTree}: nenhum dos atributos é \texttt{UNIQUE},
        pelo que fará sentido adicionar indexação a ambos os atributos. Mais, faz
        sentido, claro, manter a indexação igual para ambos os atributos, visto que
        estão ambos presentes na mesma comparação. Considerou-se,
        aqui, que \textit{Merge Join} ajudaria mais a agilizar a \textit{query}
        que \textit{Hash Join}.
\end{itemize}
...
Escolheu-se criar os índices pela ordem acima apresentada: primeiro os índices associados

...
\begin{document}
  ...
  \begin{itemize}[nosep]
    ...
    \item \textsf{\textbf{(IC-11)}} serial\_number, manuf, nr \textsf{must exist in one of} warm\_shelf \textsf{or}\\ ambient\_temperature\_shelf \textsf{or} cold\_shelf
    \item \textsf{\textbf{(IC-12)}} serial\_number, manuf, nr \textsf{never exists in more than one of} warm\_shelf \textsf{and}\\ ambient\_temperature\_shelf \textsf{or} cold\_shelf \textsf{simultaneously}
  \end{itemize}
  ...
  \begin{itemize}[nosep]
    ...
    \item \textsf{\textbf{(IC-6)} The product with} ean \textsf{must have} (has) \textsf{one of the categories} (category) \textsf{that its} retailer \textsf{, identified by} tin\textsf{, is} responsible\_for
    \end{itemize}
  ...
  \begin{enumerate}
    ...
    \[
      \begin{aligned}
        ...
        &l \leftarrow \rho_{(1->\op{total\_sum})} (_{\op{ean,descr}} G_{sum(units)}(r))\\
        &\pi_{\op{ean},~\op{descr}}\left(
          l \bowtie (\rho_{(1->\op{max})} ( G_{max(total\_sum)}(l)) )
        \right)
        \end{aligned}
    \]
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    % TODO acho que não dá para usar natural join aqui
    ...
    % TODO can we skip join with planogram?

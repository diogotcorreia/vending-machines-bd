...
DROP TRIGGER IF EXISTS self_contained_category_trigger
  ON has_other;
...
BEGIN
  IF EXISTS(
    WITH RECURSIVE list_recurs(super_category, category) AS (
      ...
      FROM has_other AS child
        INNER JOIN list_recurs AS parent
          ON child.super_category = parent.category
    )
  ...
  THEN
    RAISE EXCEPTION 'Category "%" cannot be contained within itself',
      NEW.category;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER self_contained_category_trigger
  BEFORE INSERT ON has_other
FOR EACH ROW
  EXECUTE PROCEDURE self_contained_category_trigger();
